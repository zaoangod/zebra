...
export function createWretchTests(ctx: TestContext): void {
  ...
  describe("Wretch", function () {
    ...
    it("should convert wretch instance to fetch-like function", async function () {
      const fetchFn = wretch(_URL).toFetch()
      const response = await fetchFn("/text", { method: "GET" })
      expect(response.ok).toBe(true)
      const text = await response.text()
      expect(text).toBe("A text string")
    })

    it("should preserve catchers when converting to fetch", async function () {
      ...
      await fetchFn("/404", { method: "GET" })
      expect(catcherCalled).toBe(true)
    })
    ...
    it("should merge options correctly in toFetch", async function () {
      ...
      expect(actualHeaders["X-Custom-Header"]).toBe("base")
      expect(actualHeaders["X-Custom-Header-2"]).toBe("input")
      expect(actualHeaders["X-Custom-Header-3"]).toBe("fetch")
    })

    it("should handle POST requests with body in toFetch", async function () {
      const fetchFn = wretch(_URL).toFetch()
      const body = { test: "data" }
      const response = await fetchFn("/json/roundTrip", {
        method: "POST",
    ...
    it("should handle different HTTP methods in toFetch", async function () {
      const fetchFn = wretch(_URL).toFetch()

      const getResponse = await fetchFn("/text", { method: "GET" })
      expect(getResponse.ok).toBe(true)

      const postResponse = await fetchFn("/json/roundTrip", {
        method: "POST",
      ...
      const deleteResponse = await fetchFn("/text", { method: "DELETE" })
      expect(deleteResponse.ok).toBe(true)
    })

    it("should prevent the fetch function generated by toFetch from throwing", async function () {
      const fetchFn = wretch(_URL).toFetch()
      const response = await fetchFn("/418", { method: "GET" })
